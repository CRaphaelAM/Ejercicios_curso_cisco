...
def main() -> None:
    ...
    try:
        with open (file,"r") as in_file:
            ...
            for line in in_file:
                line = line.split()
                estudiantes["nombre"] = line[0]
                estudiantes["apellido"] = line[1]
                estudiantes["nota"] = line[2]
                lista_estudiantes.append(estudiantes)
            aux = []
            for i in range(lista_estudiantes):
                if elemento["nombre"] == lista_estudiantes[i]["nombre"] and elemento["apellido"] == lista_estudiantes[i]["apellido"]:
                        aux.append(e)
